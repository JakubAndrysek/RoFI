...
struct JointController {
  JointController(rofi::hal::Joint j) : j(j) { move(); }
...
extern "C" void app_main() {
  ...
  if (std::string(SSID) == "wlan_fi")
    esp::authWlanFi(WLAN_USER, WLAN_PASS);
  ...
  esp::SocketServer server;
  server.setClientRoutine([&](esp::SocketClient client, sockaddr_in addr) {
    try {
