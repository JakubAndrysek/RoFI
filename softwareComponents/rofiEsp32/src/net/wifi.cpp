 ...
 * labmda.
 */
template <typename F> int httpLibCallbackInvoker(esp_http_client_event_t *evt) {
 ...
 * via f
 */
template <typename F> auto makeHttpDataCallback(F f) {
...
std::vector<std::string> getWlanFiAuthLinks(const char *username,
                                            const char *password) {
  ...
  esp::LineReader lineReader;
  lineReader.bind(extractLoginLink);

  auto httpCallback = makeHttpDataCallback(
      [&](char *data, int len) { lineReader.push(data, len); });
...
void wlanFiFollowAuthLink(const std::string &link, const char *username,
                          const char *password) {
...
void esp::authWlanFi(const char *username, const char *password) {
