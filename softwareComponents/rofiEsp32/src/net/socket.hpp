...
public:
  ...
  SocketClient(SocketClient &&other) { swap(other); }
   ...
   */
  static SocketClient take(int fd) { return SocketClient(fd); }
...
public:
  ...
  void bind(int port, int connectionLimit = 5) {
    ...
    int err =
        ::bind(_fd, reinterpret_cast<sockaddr *>(&sockAddr), sizeof(sockAddr));
    if (err < 0)
      throw std::runtime_error(strerror(errno));
  ...
  template <typename F> void setClientRoutine(F routine) {
    _onClient = routine;
  }
  ...
  void acceptRoutine(int fd) {
    ...
    while (true) {
      try {
        ...
        int connection =
            accept(fd, reinterpret_cast<sockaddr *>(&sockAddr), &addrLen);
